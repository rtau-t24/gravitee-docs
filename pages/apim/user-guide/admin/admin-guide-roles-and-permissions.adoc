...
What you can do on those screens are dirven by your role for the scope `MANAGEMENT`.

*A user has only one `MANAGEMENT` role and one `PORTAL` role.*
...
That's why it is a role with the lowest permissions.
...
To do that, click on the (+) icon in the table header and fill the name and the description of the new role
